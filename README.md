...
1. **EDA** — understand the target imbalance and how client attributes relate to
   subscription likelihood (age, job, education, month/day of contact, etc.).
2. **Feature engineering** — bucket `day` into beginning/middle/end-of-month, bucket
...
3. **Two-stage modelling**, rather than one model on every feature at once:
...
4. **Class imbalance** is handled via combined oversampling (minority class) and
...
5. **Threshold selection** — the operating decision threshold is chosen via the
...
6. **Campaign simulation** — for clients who clear Stage 1, the pipeline simulates
...
| Metric | Value |
|---|---|
| Threshold (validation-selected) | 0.62 |
| Test precision | 0.427 |
| Test recall | 0.421 |
| Baseline duration per subscription | 2195.7s |
| Model duration per subscription | 662.6s |
